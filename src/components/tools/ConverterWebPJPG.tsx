...
export default function ConverterWebPJPG() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && file.type === 'image/webp') {
      ...
      alert('Por favor, selecione um arquivo WebP válido');
    }
  };
  ...
  const downloadConverted = () => {
    ...
    const fileName = selectedFile?.name.replace('.webp', '.jpg') || 'converted.jpg';
    ...
    <div className="bg-white rounded-xl shadow-lg p-8">
      ...
      {originalPreview && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <h3 className="font-bold mb-2">Original (WebP)</h3>
            <div className="bg-gray-50 rounded-lg p-4">
              <img src={originalPreview} alt="Original WebP" className="w-full h-auto rounded" />
            </div>
          </div>
          {convertedPreview && (
            <div>
      ...
      <div className="mt-8 text-sm text-gray-600 bg-blue-50 border-l-4 border-blue-400 p-4">
        <p className="font-semibold mb-2">💡 Sobre WebP:</p>
        <p className="mb-2">
          WebP é um formato de imagem moderno desenvolvido pelo Google que oferece compressão
          superior ao JPG e PNG. No entanto, nem todos os softwares suportam WebP.
        </p>
        <p>
          Use esta ferramenta para converter WebP para JPG quando precisar de maior compatibilidade.
